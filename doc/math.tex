...
\setlength{\parindent}{0.5in}
{\bf Dong reply:}
A joining network of tuples is not total if any tuple is removed.
\setlength{\parindent}{0.25in}
...
\setlength{\parindent}{0.5in}
{\bf Dong reply:}
At my first thinking:

For the sufficience comes from the expressing ability of the QL language is as much as SQL queries, or at least as much as User requirements.

Let's assumpt that we got all the Candinate Join Tree, which should have all the possible single SQL solutions. Here remind the Nest SQL queries, currently we don't have nest QL, so there might some expressing are not able to achive in QL. Or if there a proof that all Nest SQL query could conver into one single SQL query, that could make difference.

Next comes the ambiguities, weight assignment is the way to assign a unique id to each path solution(or single SQL solution). 

If that assignments always capable to generate unique id for everyone ? we could do a math proof.

Will those single SQL solutions are getting same result ? if not then we have ambiguities per se, which might only be solved by mulitple SQL query or a Nest SQL query.
